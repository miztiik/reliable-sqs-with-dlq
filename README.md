...
Ocassionally messages cannot be processed for a of a variety of possible issues. In most cases, we do not know the issue. But here we know that the missing `store_id` is the cause of the issue.  When ever we encounter an message without this _field_ then we will need to the move message from the main queue to another special queue: `dead-letter queue(DLQ)`. 

Amazon SQS supports _dead-letter queues_, But Amazon SQS does not create the dead-letter queue automatically. You must first create the queue before using it as a dead-letter queue. Once you have messages in the DLQ, 
...
The incoming messages frome the store looks something like this.
